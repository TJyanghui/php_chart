...
//        mysql_unbuffered_query("set names utf8");
	...
	if(!isset($err))
	{
		...
		if(isset($_GET['sql_charset'])) $sql_charset = $_GET['sql_charset'];
		if(isset($_POST['sql_charset'])) $sql_charset = $_POST['sql_charset'];
...
//			echo $sql_chart;
		...
		$cost[1] = microtime(true) - $cost[1];
	}

	// 结果数据入数组(按行)
	if(!isset($err))
	{
		$cost[2] = microtime(true);
		...
		$cost[2] = microtime(true) - $cost[2];
	}

	// 结果数据入数组(按列)&画表
	if(!isset($err))
	{
		$cost[3] = microtime(true);
	...
	// 显示耗时
	$showcost = false;
	if(isset($_GET['cost']) && $_GET['cost']==1) $showcost = true;
	...
	$myPicture->setFontProperties(array("FontName"=>"./fonts/simsun.ttc","FontSize"=>$font_t,"R"=>0,"G"=>0,"B"=>0));
...
//		$myPicture->drawRectangle(1,0,$width-1,$height-1,array("R"=>0,"G"=>0,"B"=>0));
	...
	{
		if(isset($palette) && isset($palette['bgcolor'])) $bgcolor=$palette['bgcolor'];
...
/*
	if($showcost)
	{
		$costtitle = round($cost[0]*1000,1) . 'ms, ' . round($cost[1]*1000,1) . 'ms, ' .round($cost[2]*1000,1) . 'ms, ' .round($cost[3]*1000,1) . 'ms';
		$myPicture->drawText($width-$marg-2,$marg+2, $costtitle, array("FontSize"=>8,"Align"=>TEXT_ALIGN_TOPRIGHT, "BorderOffset"=>5));
	}
*/
	...
	}else
	{
		...
		if($chartType=='pie3d' or $chartType=='pie2d')
		{
			...
			// 画2D饼图
			if($chartType == 'pie2d') $PieChart->draw2DPie($width*0.4,$height*0.53,array("Radius"=>min($height,$width*0.7)*0.3,"Border"=>TRUE,"ValueR"=>255,"ValueG"=>255,"ValueB"=>255,"ValueAlpha"=>100,"LabelStacked"=>FALSE) + $pieSettings);
			// 画3D饼图
		...
		else
		{
			...
			while(list($idx,$field) = each($arrFields))
			{
				if(!isset($chart_cols) or $idx == 0 or in_array($idx,$chart_cols))
				{
					...
					else if(isset($chart_cols2) and in_array($idx,$chart_cols2))
					{
						...
						$myData->setSerieDescription($field,"*$field");
					}
					$nSerie += 1;
				}
			}
			...
			$myPicture->setFontProperties(array("FontName"=>"./fonts/simsun.ttc","FontSize"=>$font));
			$width_offset = 0;
			$height_offset = 0;
			if($legend == 1) 
			{
				...
				$myPicture->drawLegend($width-$marg-$widthLegend,$marg+$font_t*3,array("Style"=>LEGEND_BOX,"Mode"=>LEGEND_VERTICAL,"Alpha"=>10,"Margin"=>4,"R"=>255, "G"=>255, "B"=>255));
				$width_offset += max($widthLegend,50)+$marg;
			}
			if($legend == 2) 
			{
				$sizeLegend = $myPicture->getLegendSize(array("Mode"=>LEGEND_HORIZONTAL));
				$myPicture->drawLegend(max($width*0.5-$sizeLegend['Width']*0.5,$marg),$height-$marg-$sizeLegend['Height']*0.5,array("Style"=>LEGEND_BOX,"Mode"=>LEGEND_HORIZONTAL,"Alpha"=>10,"Margin"=>4,"R"=>255, "G"=>255,"B"=>255));
				$height_offset += $sizeLegend['Height'];
			}
			...
			$myPicture->setGraphArea($marg+18+$font*3,$marg+$font_t*3,$width-$marg-$width_offset,$height-$marg-18-$font*3-$height_offset);
//			$myPicture->setGraphArea(28+$font*3,11+$font_t*3,$width-15-$width_offset,$height-23-$font*3);
			...
			if($labelSkip == -1) $labelSkip = round($myData->getSerieCount($abscissa)/(($width-$marg-$width_offset)/30));
	...
	$cost[4] = microtime(true) - $cost[4];

	if($showcost)
	{
		//$costtitle = round($cost[0]*1000,1) . 'ms, ' . round($cost[1]*1000,1) . 'ms, ' .round($cost[2]*1000,1) . 'ms, ' .round($cost[3]*1000,1) . 'ms, ' .round($cost[4]*1000,1) . 'ms';
		$costtitle = round($cost[1]*1000,1) . 'ms';
		$myPicture->drawText($width-$marg-2,$marg+2, $costtitle, array("FontSize"=>8,"Align"=>TEXT_ALIGN_TOPRIGHT, "BorderOffset"=>5));
	}
